// Source: Adapted from Ericsson's reference ETCPACK decompression cxx logic (etcdec.cxx)
...
export function decompressColor(R_B, G_B, B_B, colors_RGB444, colors) {
...
export function calculatePaintColors59T(d, p, colors, possible_colors) {
...
function decompressBlockTHUMB59Tc(block_part1, block_part2, img, width, height, startx, starty, channels) {
    ...
    for (var x = 0; x < BLOCKWIDTH; ++x) {
        for (var y = 0; y < BLOCKHEIGHT; ++y) {
            ...
            img[channels * ((starty + y) * width + startx + x) + R] = CLAMP(0, paint_colors[block_mask[x][y]][R], 255);
            img[channels * ((starty + y) * width + startx + x) + G] = CLAMP(0, paint_colors[block_mask[x][y]][G], 255);
            img[channels * ((starty + y) * width + startx + x) + B] = CLAMP(0, paint_colors[block_mask[x][y]][B], 255);
        }
    }
}

export function decompressBlockTHUMB59T(block_part1, block_part2, img, width, height, startx, starty) {
    decompressBlockTHUMB59Tc(block_part1, block_part2, img, width, height, startx, starty, 3);
}

export function calculatePaintColors58H(d, p, colors, possible_colors) {
...
export function decompressBlockTHUMB58H(block_part1, block_part2, img, width, height, startx, starty) {
...
export function decompressBlockPlanar57(compressed57_1, compressed57_2, img, width, height, startx, starty) {
...
export function decompressBlockDiffFlip(block_part1, block_part2, dstImage, width, height, startx, starty) {
...
export function decompressBlockETC2(block_part1, block_part2, img, width, height, startx, starty) {
    decompressBlockETC2c(block_part1, block_part2, img, width, height, startx, starty, 3);
}

function getbit(byte, pos, shift) {
    var bit = (byte >> pos) & 1;
    return bit << shift;
}

function decompressBlockAlphaC(data, srcoff, dst, offset, width, height, ix, iy, channels) {
    ...
    for (var x = 0; x < 4; x++) {
        for (var y = 0; y < 4; y++) {
            ...
            var windex = channels * (ix + x + (iy + y) * width) + offset;
            var val = alphaTable[alpha][index] * mul;
            dst[windex] = CLAMP(0, alpha + val, 255);
        }
    }
}

function get16bits11signed(base, table, mul, index) {
    var tabVal = alphaBase[table][3 - index % 4];
    var sign = (1 - Math.floor(index / 4)) > 0 ? true : false;
    if (sign) {
        tabVal = tabVal + 1;
    }
    ...
    var elevenbits = base + elevenTabVal;
    if (elevenbits >= 1024) {
        elevenbits = 1023;
    } else if (elevenbits < -1024) {
        elevenbits = -1024;
    }
    return elevenbits;
}
...
function decompressBlockAlpha16bitC(data, srcoff, dst, dstoff, width, height, ix, iy, channels) {
    ...
    for (var x = 0; x < 4; x++) {
        for (var y = 0; y < 4; y++) {
            ...
            if (formatSigned) {
                const mul = new Int32Array([Math.floor(table[0] / 16)]);
                const tabled = new Int32Array([Math.floor(table[0] % 16)]);
...
export function decodeETC(src, width, height, etcFormat, forceRGBorRGBA) {
    ...
    switch (etcFormat) {
        ...
        case ETC1_RGBA8:
            dstChannelBytes = 1;
            dstChannels = 3;
            format = etcFormat == ETC1_RGBA8 ? "RGB_A" : "RGB";
            check_size(width, height, 8, src);
            break;
        case ETC2_RGBA8:
            dstChannelBytes = 1;
    ...
    if (alphaFormat == AF_11BIT) {
        for (y = 0; y < height / 4; y++) {
            for (x = 0; x < width / 4; x++) {
                ...
                src_off += 8;
                if (etcFormat == EAC_RG11 || etcFormat == EAC_RG11_SIGNED) {
    ...
    if (!(height == active_height && width == active_width)) {
        ...
        const newimg = malloc(src, dstPixelBytes * active_width * active_height);
        var xx, yy;
        var zz;
    ...
    if (forceRGBorRGBA) {
        const forcedPixelBytes = forceRGBorRGBA == RGBA ? 4 : 3;
        switch (format) {
            case "RG":
                var newimg = malloc(src, forcedPixelBytes * width * height);
            ...
            case "RGB":
                ...
                break;
            case "RGB_A":
                var newimg = malloc(src, forcedPixelBytes * width * (height / 2));
